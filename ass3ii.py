...
# Player dimensions
PLAYER_SCALE = 1.5
HEAD_RADIUS = 10 * PLAYER_SCALE
BODY_WIDTH = 20 * PLAYER_SCALE
BODY_HEIGHT = 30 * PLAYER_SCALE
BODY_DEPTH = 10 * PLAYER_SCALE
ARM_RADIUS = 5 * PLAYER_SCALE
ARM_LENGTH = 20 * PLAYER_SCALE
LEG_RADIUS = 7 * PLAYER_SCALE
LEG_LENGTH = 25 * PLAYER_SCALE
NECK_RADIUS = 0 * PLAYER_SCALE
NECK_LENGTH = 0 * PLAYER_SCALE
...
def draw_player():
    ...
    glColor3f(0, 0, 0)
    glPushMatrix()
    glTranslatef(0, 0, BODY_HEIGHT + NECK_LENGTH / 2)
    glRotatef(90, 1, 0, 0)
    gluCylinder(gluNewQuadric(), NECK_RADIUS, NECK_RADIUS, NECK_LENGTH, 8, 2)
    glPopMatrix()

    # Draw head (sphere)
    glColor3f(0, 0, 0)
    glPushMatrix()
    glTranslatef(0, 0, BODY_HEIGHT + NECK_LENGTH + HEAD_RADIUS)
    ...
    glPushMatrix()
    glTranslatef(BODY_WIDTH / 2 + ARM_RADIUS, 0, BODY_HEIGHT - ARM_LENGTH / 2)
    glRotatef(20, 0, 1, 0)
    glRotatef(90, 1, 0, 0)
    gluCylinder(gluNewQuadric(), ARM_RADIUS, ARM_RADIUS * 0.8, ARM_LENGTH, 8, 2)
    glPopMatrix()

    # Draw middle arm (cone)
    glColor3f(0.8, 0.8, 0.8)
    glPushMatrix()
    glTranslatef(0, 0, BODY_HEIGHT - ARM_LENGTH / 2)
    glRotatef(0, 0, 1, 0)
    glRotatef(90, 1, 0, 0)
    quadric = gluNewQuadric()
    gluCylinder(quadric, ARM_RADIUS*1.5, 0.0, ARM_LENGTH*1.5, 8, 2)
    glPopMatrix()
    ...
    # Draw right leg (cylinder)
    glColor3f(0, 0, 1)
    glPushMatrix()
    glTranslatef(BODY_WIDTH / 4, 0, 0)
    glRotatef(90, 0, 0, 1)
    gluCylinder(gluNewQuadric(), LEG_RADIUS, LEG_RADIUS * 0.6, LEG_LENGTH, 8, 2)
    glPopMatrix()

    # Draw left leg (cylinder)
    glColor3f(0, 0, 1)
    glPushMatrix()
    glTranslatef(-BODY_WIDTH / 4, 0, 0)
    glRotatef(90, 0, 0, 1)
