...
cheat_vision = False
cheat_mode = False
cheat_unlimited_health = False   # Cheat 1: unlimited HP
cheat_aimbot = False             # Cheat 2: aimbot
...
def keyboardListener(key, x, y):
    ...
    global current_bullet_color, cheat_unlimited_health, cheat_aimbot
    ...
    # Movement, rotation, cheat mode & vision
    if key == b's':
        fx, fy = forward_xy(-player_angle)
        ...
        cheat_unlimited_health = not cheat_unlimited_health
    elif key == b'h':
        cheat_aimbot = not cheat_aimbot
...
def check_collisions():
    ...
    for i, bullet in enumerate(bullets):
        bx, by, _, _, bullet_color = bullet
        for enemy in enemies:
            ex, ey, _, scale, _, eye_color = enemy

            # Use distance check
            if math.hypot(bx - ex, by - ey) < 35:
                if bullet_color == eye_color:
                    # Colors match → enemy dies
                    player_score += 1
                    ...
                    enemy[3] = 1.0  # reset scale
                    enemy[5] = random.choice(MILD_EYE_COLORS)
                    to_remove.append(i)
                else:
                    # Colors mismatch → double the enemy's scale
                    enemy[3] *= 2
                    # Remove bullet anyway
                    to_remove.append(i)
                break
    ...
    for enemy in enemies:
        if math.hypot(enemy[0] - player_pos[0], enemy[1] - player_pos[1]) < 20:
            player_life -= 1
            enemy[0] = random.randint(-ENEMY_RESPAWN_RADIUS, ENEMY_RESPAWN_RADIUS)
            enemy[1] = random.randint(-ENEMY_RESPAWN_RADIUS, ENEMY_RESPAWN_RADIUS)
            enemy[5] = random.choice(MILD_EYE_COLORS)
...
def idle():
    global player_angle, game_over, player_life, bullets

    if game_over:
        glutPostRedisplay()
        return

    # Cheat 1: Unlimited health
    if cheat_unlimited_health:
        player_life = 9999

    # Cheat 2: Aimbot
    if cheat_aimbot:
        # find closest enemy with matching color
        target_enemy = None
        min_dist = float('inf')
        for enemy in enemies:
            ex, ey, _, _, _, eye_color = enemy
            if eye_color == current_bullet_color:
        ...
        if target_enemy:
            dx = target_enemy[0] - player_pos[0]
            dy = target_enemy[1] - player_pos[1]
            angle_to_enemy = math.degrees(math.atan2(dx, dy))
            player_angle = -angle_to_enemy + 180
            fx, fy = forward_xy(player_angle)
            spawn_x = player_pos[0] + fx * BULLET_SPAWN_OFFSET
            spawn_y = player_pos[1] + fy * BULLET_SPAWN_OFFSET
