...
# Forward vector matching glRotatef(+yaw around +Z) when model's local +Y is "forward"
# Local +Y rotated by yaw -> world (-sin(yaw), cos(yaw))
...
def muzzle_world():
    ...
    fx, fy = yaw_forward(player.yaw_deg)        # forward (+Y when yaw=0)
...
def yaw_face_point(px, py, tx, ty):
    """Yaw angle (deg) so that yaw_forward(yaw) points from (px,py) -> (tx,ty)."""
    dx, dy = tx - px, ty - py
    if abs(dx) < 1e-6 and abs(dy) < 1e-6:
        return player.yaw_deg
    # Because yaw_forward(yaw) = (-sin r, cos r), we want sin r = -ux, cos r = uy
...
def fire_bullet(now, target=None):
    ...
    # If aimbot is on and we have a target, rotate player to face it so muzzle matches the shot
...
def update_world(dt, now):
    ...
    # aimbot: face target before firing so muzzle matches bullet
    if player.aimbot and not game_paused and not game_over:
        ...
        if tgt is not None and tgt.alive:
            # snap face the target, then fire
    ...
    if button == GLUT_LEFT_BUTTON and state == GLUT_DOWN:
        ...
        if not game_over and not game_paused:
            if player.aimbot:
                tgt, _ = nearest_enemy_to(player.x, player.y)
                if tgt is not None:
                    # face target first so muzzle aligns
                    player.yaw_deg = yaw_face_point(player.x, player.y, tgt.x, tgt.y)
