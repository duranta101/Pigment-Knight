...
def yaw_forward(yaw_deg):
    ...
    return math.sin(r), math.cos(r)  # +Y is "forward" at yaw 0
...
# Model (base dims); +Y is forward at yaw 0
STEVE_SCALE = 2.0
...
# Blaster offsets relative to player local axes
...
def draw_steve_player():
    ...
    # body
    glColor3f(0.05, 0.05, 0.05)  # dark suit
    glPushMatrix()
    glScalef(12, 8, 18)
    glutSolidCube(1)
    glPopMatrix()
...
def setup_camera():
    ...
    if camera_mode == 0:
        ...
        # First-person from just behind player's head, pointed along yaw
...
def keyboardListener(key, x, y):
    ...
    if k == b'w':
        fx, fy = yaw_forward(player.yaw_deg)
        player.x = clamp(player.x + fx * PLAYER_STEP, -LENGTH_OF_GRID*0.95, LENGTH_OF_GRID*0.95)
        player.y = clamp(player.y + fy * PLAYER_STEP, -LENGTH_OF_GRID*0.95, LENGTH_OF_GRID*0.95)
    if k == b's':
        fx, fy = yaw_forward(player.yaw_deg)
        player.x = clamp(player.x - fx * PLAYER_STEP, -LENGTH_OF_GRID*0.95, LENGTH_OF_GRID*0.95)
        player.y = clamp(player.y - fy * PLAYER_STEP, -LENGTH_OF_GRID*0.95, LENGTH_OF_GRID*0.95)
    if k == b'a':
        player.yaw_deg += ROT_STEP_DEG
    if k == b'd':
        player.yaw_deg -= ROT_STEP_DEG
